...
class FlightBooking(Base):
    """A flight booked through a Duffel Links hosted checkout session."""
    __tablename__ = "flight_bookings"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    duffel_order_id: Mapped[str] = mapped_column(String(50), nullable=False, unique=True)
    booking_reference: Mapped[str | None] = mapped_column(String(20), nullable=True)  # airline PNR
    ...
    depart_time: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    total_amount: Mapped[Decimal | None] = mapped_column(Numeric(10, 2), nullable=True)
    currency: Mapped[str] = mapped_column(String(5), default="USD")
    passengers: Mapped[list[str] | None] = mapped_column(ARRAY(String), nullable=True)
...
class PriceSnapshot(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    flight_watch_id: Mapped[uuid.UUID] = mapped_column(sa.ForeignKey("flight_watches.id", ondelete="CASCADE"), nullable=False)
